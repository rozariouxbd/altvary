...
/** Customer ids suppressed for this store (excluded from every play). */
async function suppressedCustomerIds(storeId: string): Promise<string[]> {
...
/** Evaluate every registered play (powers the recommendations list/board page). */
export async function evaluateAll(store: Store): Promise<PlayEvalResult[]> {
